...
void ska::ScriptWorldTriggerSystem::refresh(unsigned int) {
	...
	for(auto& entityId : processed) {
		...
		const auto lastBlock = m_world.getCollisionProfile().getHighestNonCollidingBlock(oldPos.x / blockSize, oldPos.y / blockSize);
		const auto newBlock = m_world.getCollisionProfile().getHighestNonCollidingBlock(centerPos.x / blockSize, centerPos.y / blockSize);
