...
namespace ska {
	...
	protected:
		...
		virtual std::string execute(ScriptComponent& script, MemoryScript& memory, std::vector<std::string>& args) = 0;

		std::string process(ScriptComponent& script, MemoryScript& memory, std::stringstream& streamCmd, std::vector<std::string>& args) override;
